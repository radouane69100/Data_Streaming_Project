...
# définition de la fonction pour traiter les messages
def process_msg_data_clean(chan: BlockingChannel, method: Basic.Deliver, properties: BasicProperties, body):
    body = body.decode("utf-8")      # décodage du corps du message en UTF-8
    ...
    # fonction pour trouver une correspondance avec l'expression régulière
    ...
    if match != None:
        #retourne un identifiant unique en hashant la chaîne de caractères body avec l'algorithme MD5
        def get_id(body):
        ...
        def get_day_of_week(match):
            ...
            return day_of_week
        #Extraction de l'heure de la requete
        def get_time(match):
            date_time = get_timestamp(match)
        ...
        if get_user(match) != None or get_session(match) != None:
            ...
            # Crée un objet CleanLog en utilisant les fonctions pour extraire les informations du message et les stocke dans l'objet.
...
# Consume le message à partir de la queue "queue-data-clean" et appelle la fonction process_msg_data_clean pour traiter chaque messag
...
#Démarrage du canal pour la consommation
